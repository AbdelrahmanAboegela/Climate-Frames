...
from climate_frames.dataset import (
    DEFAULT_DATA_PATH,
    dataset_profile,
    load_annotations,
...
def run_lora_cv(df: pd.DataFrame, label2id: dict[str, int], splits, epochs: int = 4):
    ...
    for fold, (train_idx, val_idx) in enumerate(splits, start=1):
        ...
        args = TrainingArguments(
            output_dir=os.path.join(OUTPUT_DIR, f"core_lora_fold{fold}"),
...
def plot_per_class_comparison(results_by_name: dict[str, tuple[list[int], list[int]]], id2label):
    ...
    ax.set_title("Core-frame classification on merged current workbook - per-class F1")
